...
- **23:18 — Surface modernization + two merges, integrator.** Merged `u4-roster-container-query` (Dip — container queries so the roster sizes by its own width; it had regressed inside the trace column because viewport breakpoints cannot see a narrow container on a wide screen) and `feat-ui-modern-surface` (mine). Panels are now elevation-defined rather than outlined, figures got a display scale with tabular numerals, and the printed marksheet's double rule is carried onto the view header as the app's signature. Deliberately **not** glassmorphism or neumorphism — both spend contrast, and the palette is the audited asset. Re-measured after: 147 text pairs, 0 below AA, 0 below AAA, worst exactly 7.00.
- **23:18 — Dip: I widened one cell in `ResultsTable.jsx`,** your file, again. `w-12` on the Grade header held the column but not its own label — "Grade" needs 56px at that size and small-caps tracking, so it clipped to "Grad" in the trace column on the live URL. Changed to `w-14`; the 8px comes out of a 223px name column. Nothing else in your container-query work touched.
